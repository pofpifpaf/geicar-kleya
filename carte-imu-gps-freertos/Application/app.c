 ...
 */
void APP_Init(void) {
	COM_Init(&huart2, huart2.hdmarx); // demarre la reception circulaire sur USART2
	SENSORS_Init();
...
void APP_Run(AppMessage_typeDef *msg) {
	...
	case MOTION_SEND_MEASURES_ID:
		...
		if (motion_data_msg != NULL) {
			...
			motion_data_msg->header.id = COM_MSG_MOTION_ID;
	...
	case ENV_SEND_MEASURES_ID:
		...
		if (env_data_msg != NULL) {
			...
			env_data_msg->header.id = COM_MSG_ENV_ID;
	...
	case ECOMPASS_SEND_MEASURES_ID:
		...
		if (ecompass_data_msg != NULL) {
			...
			ecompass_data_msg->header.id = COM_MSG_ECOMPASS_ID;
