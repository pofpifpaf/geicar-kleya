...
static uint32_t SENSORS_LSM6DSV16X_InitAcc(LSM6DSV16X_Object_t *handler);
...
static uint32_t SENSORS_LSM6DSV16X_InitGyro(LSM6DSV16X_Object_t *handler);
static void SENSORS_CalibrateGyroBias(LSM6DSV16X_Object_t *pObj);

/* Magnetometer LIS2MDL */
LIS2MDL_Object_t lis2mdl_mag_handler;
LIS2MDL_AxesRaw_t magnetic_field_raw = {0};
float lis2mdl_mag_sensitivity = 0.0f;
static uint32_t SENSORS_LIS2MDL_InitMag(LIS2MDL_Object_t *handler);

/* Pressure sensor LPS22DF */
LPS22DF_Object_t lps22df_handler;
static uint32_t SENSORS_LPS22DF_Init(LPS22DF_Object_t *handler);

/* Humidity sensor SHT40AD1B */
SHT40AD1B_Object_t sht40ad1b_handler;
static uint32_t SENSORS_SHT40AD1B_Init(SHT40AD1B_Object_t *handler);

/* Temperature sensor STTS22H */
STTS22H_Object_t stts22h_handler;
static uint32_t SENSORS_STTS22H_InitTemp(STTS22H_Object_t *handler);
 ...
 * This function initializes the LIS2MDL magnetometer sensor.
 *
 * @param handler Pointer to the LIS2MDL object handler.
 * @return uint32_t SENSOR_OK if initialization is successful, SENSOR_ERROR otherwise.
 */
static uint32_t SENSORS_LIS2MDL_InitMag(LIS2MDL_Object_t *handler)
{
	LIS2MDL_IO_t io_ctx;
...
/**
 * @brief  Initialize the LSM6DSV16X accelerometer.
 * This function initializes the LSM6DSV16X accelerometer sensor.
 *
 * @param handler Pointer to the LSM6DSV16X object handler.
 * @return uint32_t SENSOR_OK if initialization is successful, SENSOR_ERROR otherwise.
 */
static uint32_t SENSORS_LSM6DSV16X_InitAcc(LSM6DSV16X_Object_t *handler)
{
...
/**
 * @brief  Initialize the LSM6DSV16X gyroscope.
 * This function initializes the LSM6DSV16X gyroscope sensor.
 *
 * @param handler Pointer to the LSM6DSV16X object handler.
 * @return uint32_t SENSOR_OK if initialization is successful, SENSOR_ERROR otherwise.
 */
...
/**
 * @brief  Initialize the LPS22DF pressure sensor.
 * This function initializes the LPS22DF pressure sensor.
 *
 * @param handler Pointer to the LPS22DF object handler.
 * @return uint32_t SENSOR_OK if initialization is successful, SENSOR_ERROR otherwise.
 */
static uint32_t SENSORS_LPS22DF_Init(LPS22DF_Object_t *handler) {
	LPS22DF_IO_t            io_ctx;
...
/**
 * @brief  Initialize the SHT40AD1B humidity sensor.
 * This function initializes the SHT40AD1B humidity sensor.
 *
 * @param handler Pointer to the SHT40AD1B object handler.
 * @return uint32_t SENSOR_OK if initialization is successful, SENSOR_ERROR otherwise.
 */
static uint32_t SENSORS_SHT40AD1B_Init(SHT40AD1B_Object_t *handler) {
...
/**
 * @brief  Initialize the STTS22H temperature sensor.
 * This function initializes the STTS22H temperature sensor.
 *
 * @param handler Pointer to the STTS22H object handler.
 * @return uint32_t SENSOR_OK if initialization is successful, SENSOR_ERROR otherwise.
 */
static uint32_t SENSORS_STTS22H_InitTemp(STTS22H_Object_t *handler) {
	STTS22H_IO_t io_ctx;
...
/**
 * @brief  Calibrate the gyroscope bias.
 * This function calculates the gyroscope bias by averaging multiple samples
 * while the sensor is stationary.
 *
 * @param pObj Pointer to the LSM6DSV16X object handler.
 */
static void SENSORS_CalibrateGyroBias(LSM6DSV16X_Object_t *pObj)
{
 ...
 * This function converts raw accelerometer data to acceleration in g.
 *
 * @param raw Pointer to the raw accelerometer data.
 * @return SENSORS_TriaxeValues_t Acceleration in g.
 */
 ...
 * This function converts raw gyroscope data to angular velocity in degrees per second.
 *
 * @param raw Pointer to the raw gyroscope data.
 * @return SENSORS_TriaxeValues_t Angular velocity in DPS.
 */
 ...
 * This function converts raw magnetometer data to magnetic field strength in milliGauss.
 *
 * @param raw Pointer to the raw magnetometer data.
 * @return SENSORS_TriaxeValues_t Magnetic field strength in milliGauss.
 */
